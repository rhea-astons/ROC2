...
/**
 * Classe SousCategoryEditor Création de la fenêtre d'éditon de sous catégories
 * 
...
public class SousCategoryEditor extends JDialog implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == btSubmit) {
			...
			if (sousCatName.getText().length() > 3 && edit && !exist) {
				sousCategorie = Roc2DB.editSousCategorie(sousCatName.getText(),
						sousCatToEdit);
				setVisible(false);
			} else if (sousCatName.getText().length() > 3 && !exist) {
				sousCategorie = Roc2DB.addSousCategorie(sousCatName.getText(),
						categorie.id);
				setVisible(false);
			} else {
				if (exist) {
	...
	/**
	 * Vérifie si une sous catégorie existe déja
	 * 
	 * @param name
	 *            nom de la sous catégorie à vérifier
